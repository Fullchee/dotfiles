...
# Load gcloud completion later from ~/.zshrc after Prezto has initialized
# the completion system. Sourcing it here triggers an extra early compinit.
...
prod-cloud-sql-proxy() {
    cloud-sql-proxy "${PROD_PROJECT}:${CLOUD_SQL_REGION}:crawler-db-v84" \
        --address 0.0.0.0 \
        --port "$CLOUD_SQL_PORT"
}
...
pt() {
    ...
    (
        ...
        for arg in "$@"; do
            ...
            if [[ "$arg" != */* ]]; then
                # Filename only: find all matching files under the repo
                while IFS= read -r found; do
                    args+=("${found#./}")
                done < <(find . -name "$arg" -type f 2>/dev/null)
            elif [[ ! -e "$arg" ]]; then
                # Partial sub-path (e.g. lists/test_serializers.py): find by suffix
                while IFS= read -r found; do
                    args+=("${found#./}")
                done < <(find . -path "*/$arg" -type f 2>/dev/null)
            else
                args+=("$arg")
            fi
        done

        docker-compose -f local-dev/docker-compose.yml run --rm django pytest -s "${args[@]}"
    )
    cd -
}
