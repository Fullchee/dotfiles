...
- use `gh` (GitHub CLI) instead of `git` when appropriate
    - set `GH_PAGER=cat` to disable paging when using `gh` commands that output to the terminal (e.g. `GH_PAGER=cat gh pr view 3412 --json number,title,body,comments,reviewRequests,reviews --repo WatrHub/watrhub-django`)
